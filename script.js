  ...
  if (config.USE_REMOTE_DATA === true && remoteUrl) {
    try {
      ...
      try {
        ...
        console.log("Loaded local data.json after remote fallback");
        return {
          data,
          sourceLabel: "Remote失敗 → Local fallback",
          warning: "Remote JSONの取得に失敗したため、data.jsonを表示しています"
        };
      } catch (localError) {
        ...
        return {
          data: null,
          sourceLabel: "読み込み失敗",
          warning: "Remote JSONとdata.jsonの両方を読み込めませんでした"
        };
      }
    }
  }
...
function renderTabs() {
  const tabs = [
    { id: "all", label: "全体" },
    ...dashboardData.stocks.map((stock) => ({ id: stock.code, label: stock.shortName || stock.name }))
  ];
...
function renderOverview() {
  ...
  return `
    <section class="card"><h2>全体サマリー</h2><div class="summary-grid">
      ${renderMetric("今日動く必要", summary.actionRequired)}
      ${renderMetric("全体方針", summary.overallPolicy)}
      ${renderMetric("要注意銘柄", summary.watchStock)}
      ${renderMetric("今週の焦点", summary.weeklyFocus)}
    </div></section>
    <section class="section-band"><h2>3銘柄まとめ</h2><div class="stock-grid">${stocks.map(renderStockSummary).join("")}</div></section>
    <section class="card"><h2>共通チェックポイント</h2>${renderList(summary.commonCheckpoints)}</section>
    <section class="card"><h2>週次レビュー用メモ</h2>${renderWeeklyReview(dashboardData.weeklyReview)}</section>
  `;
}
...
function renderStockSummary(stock) {
  return `
    <article class="card">
      <div class="badge-row">${renderBadge(stock.conclusion, "hold")}${renderBadge(`信頼度 ${stock.confidence}`, "watch")}</div>
      <h3>${escapeHtml(stock.name)}</h3>
      <p class="lead">${escapeHtml(stock.oneLine)}</p>
      <div class="metric-grid">
        ...
        ${renderMetric("注意度", stock.attentionLevel)}
      </div>
    </article>
  `;
}
  ...
  return `
    <section class="card">
      ...
      <div class="metric-grid">
        ${renderMetric("株価", formatYen(stock.price))}
        ${renderMetric("前日比", formatChange(stock.change, stock.changeRate))}
        ${renderMetric("今週動く必要", stock.actionRequired)}
        ${renderMetric("想定レンジ", stock.forecast.range)}
      </div>
    </section>
    <section class="card"><h2>今日時点の判断</h2><p>${escapeHtml(stock.todayJudgement)}</p><ul class="decision-list">${stock.decisionDetails.map((item) => `<li><span class="label">${escapeHtml(item.label)}</span>${escapeHtml(item.value)}</li>`).join("")}</ul></section>
    <section class="section-band"><h2>関連ニュース</h2><p class="lead">${escapeHtml(stock.newsSummary)}</p><div class="news-grid">${stock.relatedNews.slice(0, 3).map(renderNews).join("")}</div></section>
    <section class="card"><h2>今週の予想</h2><div class="policy-grid">${renderPolicy("強い場合", stock.forecast.strongCase)}${renderPolicy("弱い場合", stock.forecast.weakCase)}${renderPolicy("基本想定", stock.forecast.baseCase)}</div></section>
    <section class="card"><h2>今後見るポイント</h2><div class="tag-list">${stock.watchPoints.map((point) => `<span class="tag">${escapeHtml(point)}</span>`).join("")}</div></section>
    <section class="card"><h2>方針変更トリガー</h2>${renderList(stock.policyTriggers)}</section>
    <section class="card"><h2>最短の見通し</h2><p class="footer-note">${escapeHtml(stock.shortOutlook)}</p></section>
  `;
}
...
function formatChange(change, rate) {
  const sign = change > 0 ? "+" : "";
  return `${sign}${Number(change).toLocaleString("ja-JP")}円 / ${sign}${rate}%`;
}
