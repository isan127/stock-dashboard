...
function loadRemoteJsonp(url) {
  return new Promise((resolve, reject) => {
    ...
    script.onerror = () => {
      finish(reject, new Error("JSONP script load failed"));
    };

    script.src = withParams(url, {
...
function displayLoadError() {
  ...
  document.getElementById("content").innerHTML = `
    <p class="error">データを読み込めませんでした</p>
  `;
}
...
function getDashboardSubtitle(meta) {
  ...
  if (displayType && !INTERNAL_DISPLAY_TYPES.has(displayType)) {
    for (const [keyword, title] of DISPLAY_TYPE_TITLES) {
      if (displayType.includes(keyword)) return title;
    }
  }
...
function renderTabs(stocks) {
  const tabs = [
    { id: "all", label: "全体" },
    ...stocks.map((stock) => ({ id: stock.code, label: stock.shortName || stock.name }))
  ];

  document.getElementById("tabs").innerHTML = tabs.map((tab) => `
    <button class="tab-button" type="button" role="tab" aria-selected="${tab.id === activeTab}" data-tab="${escapeHtml(tab.id)}">
  ...
  return `
    <section class="card">
      ...
      <div class="summary-grid">
        ${renderMetric("今日動く必要", summary.actionRequired || summary.needAction || "なし")}
        ${renderMetric("全体方針", summary.overallPolicy || "未設定")}
        ${renderMetric("要注意銘柄", summary.watchStock || summary.alertStock || "未設定")}
        ${renderMetric("今週の焦点", summary.weeklyFocus || "未設定")}
      </div>
    </section>
    ...
    <section class="section-band">
      ...
      <div class="stock-grid">
        ${stocks.map(renderStockSummary).join("")}
      </div>
    </section>

    <section class="card">
      <h2>共通チェックポイント</h2>
      ${renderList(summary.commonCheckpoints || [])}
    </section>

    <section class="card">
      <h2>週次レビュー用メモ</h2>
      ${renderWeeklyReview(dashboardData.weeklyReview)}
    </section>
  `;
}
...
function renderStockSummary(stock) {
  return `
    <article class="card">
      <div class="badge-row">
        ${renderBadge(stock.conclusion || "未設定", "hold")}
        ${renderBadge(`信頼度 ${stock.confidence || "未設定"}`, "watch")}
      </div>
      <h3>${escapeHtml(stock.name)}</h3>
      <p class="lead">${escapeHtml(stock.oneLine || stock.summaryComment || "")}</p>
      <div class="metric-grid">
        ${renderMetric("証券コード", stock.code || "--")}
        ${renderMetric("株価", formatYen(stock.price))}
        ${renderMetric("前日比", formatChange(stock.change, stock.changeRate))}
        ${renderMetric("注意度", stock.attentionLevel || stock.priority || "未設定")}
      </div>
    </article>
  `;
}
  ...
  return `
    <section class="card">
      ...
      <div class="metric-grid">
        ${renderMetric("株価", formatYen(stock.price))}
        ${renderMetric("前日比", formatChange(stock.change, stock.changeRate))}
        ${renderMetric("今週動く必要", stock.actionRequired || stock.needAction || "なし")}
        ${renderMetric("想定レンジ", formatRange(forecast.range))}
      </div>
    </section>

    <section class="card">
      <h2>今日時点の判断</h2>
      <p>${escapeHtml(stock.todayJudgement || stock.decisionText || "基本方針をもとに確認してください。")}</p>
      <ul class="decision-list">
        ${decisionDetails.map((item) => `<li><span class="label">${escapeHtml(item.label)}</span>${escapeHtml(item.value)}</li>`).join("")}
      </ul>
    </section>

    <section class="section-band">
      <h2>関連ニュース</h2>
      <p class="lead">${escapeHtml(stock.newsSummary || stock.newsTrend || "")}</p>
      <div class="news-grid">
        ${relatedNews.slice(0, 3).map(renderNews).join("") || `<p class="footer-note">ニュースは未設定です。</p>`}
      </div>
    </section>
    ...
    <section class="card">
      ...
      <div class="policy-grid">
        ${renderPolicy("強い場合", forecast.strongCase || "")}
        ${renderPolicy("弱い場合", forecast.weakCase || "")}
        ${renderPolicy("基本想定", forecast.baseCase || forecast.forecast || forecast.prediction || "")}
      </div>
    </section>
    ...
    <section class="card">
      <h2>今後見るポイント</h2>
      <div class="tag-list">
        ${(stock.watchPoints || []).map((point) => `<span class="tag">${escapeHtml(point)}</span>`).join("") || `<span class="tag">未設定</span>`}
      </div>
    </section>
    ...
    <section class="card">
      <h2>最短の見通し</h2>
      <p class="footer-note">${escapeHtml(stock.shortOutlook || stock.shortTermView || "")}</p>
    </section>
  `;
}
...
function breakdownToList(breakdown) {
  if (!breakdown) return [];
  return [
    { label: "買い増し", value: breakdown.buy || "" },
    { label: "利確", value: breakdown.takeProfit || "" },
    { label: "保有", value: breakdown.hold || "" }
...
function renderNews(news) {
  return `
    <article class="news-item">
      <h3>${escapeHtml(news.title || "無題")}</h3>
      <p>${escapeHtml(news.content || "")}</p>
      <p class="footer-note">株価への影響: ${escapeHtml(news.impact || "未設定")} / ソース: ${escapeHtml(news.source || "未設定")}</p>
    </article>
  `;
}
...
function renderWeeklyReview(review) {
  ...
  const items = [
    ["週間結果", review.weeklyResult],
    ["月曜予想", review.mondayForecast],
    ["実際のレンジ", review.actualRange],
    ["一致度", review.matchLevel],
    ["当たった点", review.matchedPoints],
    ["外れた点", review.missedPoints],
    ["次回改善", review.nextImprovement],
    ["暫定次回方針", review.provisionalNextPolicy]
  ];
...
function renderList(items) {
  if (!items || items.length === 0) return `<p class="footer-note">未設定です。</p>`;
...
function formatChange(change, rate) {
  ...
  const rateText = rate === null || rate === undefined || rate === "" ? "--" : `${sign}${rate}%`;
  return `${sign}${Number(change).toLocaleString("ja-JP")}円 / ${rateText}`;
}
